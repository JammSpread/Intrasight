	...
	getChildren(): DocsModel[] {
		let items = [
			...
			},
			{
				label: "C#",
				websiteURL:
					"https://docs.microsoft.com/en-gb/dotnet/csharp/language-reference/",
				icon: "microsoft.png",
				collapsibleState: vscode.TreeItemCollapsibleState.None,
			},
			{
			...
			},
			{
				label: ".NET CORE",
				websiteURL:
					"https://docs.microsoft.com/en-us/dotnet/api/?view=netcore-2.2",
				icon: "microsoft.png",
				collapsibleState: vscode.TreeItemCollapsibleState.None,
			},
		]
		...
		if (items.length !== 0) {
			for (let i = 0; i < items.length; i++) {
				...
				MenuItems.push(
					new DocsModel(label, websiteURL, icon, collapsibleState, {
						command: "Docs.launch",
						title: "",
