	...
	getChildren(): Thenable<StackOverflowModel[]> {
		...
		if (this.userQuery === "" || this.userQuery === undefined) {
			...
			return new Promise((resolve, reject) => {
				...
				let numDisplayResults = config.get(
					"StackOverflowNumberOfDisplayedResults",
					15,
				)
				https.get(
					`https://api.stackexchange.com/2.2/search?page=1&pagesize=${numDisplayResults}&order=desc&sort=relevance&intitle=${query}&site=stackoverflow`,
					res => {
						let raw = ""
